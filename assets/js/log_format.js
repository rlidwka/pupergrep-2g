...
var print = (function() {
  var l, levels, max, pad, toArray;
  toArray = function toArray(enu) {
    var i, _i, _ref, _results;
    _results = [];
    for (i = _i = 0, _ref = enu.length; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
      _results.push(enu[i]);
    }
    return _results;
  };
  ...
  return print = function print(obj) {
    var type = obj.level;
	 var msg = obj.msg;
    var finalmsg;
...
module.exports.get = function(req, res, next) {
	req.org.db.collection('logs').find({}).sort({_id:-1}).limit(50).toArray(function(err, logs) {
		res.render('logs', {
			logs: logs,
			print: function(obj) {
				var str = print(obj);
  ...
  Filter = (function() {
    ...
    Filter.prototype.toHtml = function toHtml(input) {
      var buf;
    ...
    Filter.prototype.forEach = function forEach(callback) {
      var buf, handleDisplay,
        _this = this;
      buf = '';
      handleDisplay = function handleDisplay(code) {
    ...
    Filter.prototype.pushTag = function pushTag(tag, style) {
      if (style == null) {
        style = '';
      }
    ...
    Filter.prototype.pushStyle = function pushStyle(style) {
      return this.pushTag("span", style);
    };

    Filter.prototype.closeTag = function closeTag(style) {
      var last;
    ...
    Filter.prototype.resetStyles = function resetStyles() {
      var stack, _ref;
    ...
    Filter.prototype.tokenize = function tokenize(text, callback) {
      var ansiMess, length, newline, realText, remove, removeXterm256, tokens, _results1,
        _this = this;
      remove = function remove(m) {
        return '';
      };
      removeXterm256 = function removeXterm256(m, g1) {
        callback('xterm256', g1);
        return '';
      };
      newline = function newline(m) {
      ...
      ansiMess = function ansiMess(m, g1) {
        var code, _j, _len;
        if (g1.trim().length === 0) {
          g1 = '0';
        }
